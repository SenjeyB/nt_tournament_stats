...
1. Using mods that aren't listed or aren't of the same version is forbidden and willresult in disqualification. This rule does not apply to purely cosmetic mods.
2. Each participant must turn on the timer in the game settings.
3. As all matches will be streamed live on Twitch, players must be able to streamNuclear Throne without any interruptions or lag. The quality of the stream is almostirrelevant, as long as it is smooth and distinguishable.
...
6. For tournment stage 1 use `derbybo5` preset. For other ones use `derbybo7`. Pleaserefer to the pinned guide in the channel.
